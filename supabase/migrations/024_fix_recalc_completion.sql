-- 024_fix_recalc_completion.sql
-- 재계산 후 리밸런싱 완료 시 상태가 항상 'partial'이 되는 버그 수정
--
-- 문제: complete_rebalance_session이 resolved_by_recalc 주문을 전체 카운트(v_total)에
--       포함하지만, 이 주문들은 executed_quantity=0이므로 절대 v_executed에 카운트되지 않음.
--       결과적으로 재계산 후 모든 활성 주문을 체결해도 status가 항상 'partial'이 됨.
-- 수정: resolved_by_recalc 주문을 카운트 및 포트폴리오 업데이트 루프에서 제외.
--       기준: 017_fix_cash_double_conversion.sql의 complete_rebalance_session
...
BEGIN
  ...
  -- 실행된 주문 카운트 (resolved_by_recalc 주문 제외)
  SELECT count(*) INTO v_total
  FROM jsonb_array_elements(v_orders) AS elem
  WHERE COALESCE((elem->>'resolved_by_recalc')::boolean, false) = false;
  ...
  -- 상태 결정 (v_total=0: 모든 주문이 resolved된 경우 completed 처리)
  IF v_total = 0 OR v_executed >= v_total THEN
    v_status := 'completed';
  ELSE
    v_status := 'partial';
  END IF;
  ...
  IF v_portfolio_id IS NOT NULL THEN
    ...
    LOOP
      -- 재계산으로 해소된 주문은 포트폴리오 업데이트 스킵
      IF COALESCE((v_elem->>'resolved_by_recalc')::boolean, false) THEN
        CONTINUE;
      END IF;
